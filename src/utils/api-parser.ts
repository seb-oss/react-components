...
import { Declaration, TypescriptParser, ClassDeclaration, PropertyDeclaration, VariableDeclaration, InterfaceDeclaration, TypeAliasDeclaration } from "typescript-parser-es5";
...
export class APIExtractService {
    ...
    initParse(sourceFileUrl: any, importedTypeFileUrls?: Array<any>): Promise<Array<ApiSection>> {
        return this.parseSourceFile(sourceFileUrl?.default, importedTypeFileUrls || []);
    }
    ...
    async extractImportedTypesInterfaces(imports: Array<any>): Promise<Array<Array<ParsedPropertyDeclartion>>> {
        const test = imports.map((item: any) => {
            const rawSource: string = item.default;
