...
import { useMediaQuery } from "../../__utils/customHooks";
...
export const SideMenu: React.FC<SideMenuProps> = (props: SideMenuProps) => {
    ...
    const searchNavigations = React.useCallback(
    ...
    return (
        <aside className={"side-menu" + (toggle === null ? "" : toggle ? " opened" : " closed") + (isMobile ? " mobile" : "")}>
            <div
                className="toggle"
                onClick={() => {
            ...
            <div className="sidemenu-content">
                ...
                <TextBoxGroup
                    ...
                    rightIcon={
                        <svg xmlns="http://www.w3.org/2000/svg" width="24.03" height="24" viewBox="0 0 24.03 24">
                            <path
                                fillRule="evenodd"
                                d="M1221,240a9,9,0,1,1-9,9A9,9,0,0,1,1221,240Zm0,2a7,7,0,1,1-7,7A7,7,0,0,1,1221,242Zm6.71,12.282,8.01,7.984a1.011,1.011,0,0,1-1.43,1.428l-8.01-7.984A1.011,1.011,0,0,1,1227.71,254.282Z"
                                transform="translate(-1212 -240)"
                            />
                        </svg>
                    }
                />
                    ...
                    <DisplayList
                        list={listOfComponents
                            ...
                            .map((item: SideBarItem, index: number) => (
                                <NavLink className={"list-item" + (highlighted === index ? " highlighted" : "")} key={index} to={item.path} activeClassName="active" data-value={index}>
                                    {item.name}
                                </NavLink>
                            ))}
                    />
                ...
                <div className="options">
                    <h5>Dark Mode</h5>
                    <Toggle name="dark-mode" value={props.darkMode} onChange={props.onDarkModeChanged} />
                </div>
            </div>
        </aside>
    );
...
const DisplayList: React.FC<DisplayListProps> = (props: DisplayListProps) => {
    return <>{props.list.length ? props.list : <div>Nothing found</div>}</>;
};
