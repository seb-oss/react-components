...
export const Table: React.FunctionComponent<TableProps> = React.memo(
    (props: TableProps): React.ReactElement<void> => {
        const { headers, displayRows, onSort, getRow } = useTable(props.columns, props.data, { enableRowSelect: true, offset: props.offset, currentPage: props.currentpage });
        ...
        //         (e: React.ChangeEvent<HTMLInputElement>, selectedRow: TableRow, parentRow?: TableRow): void => {
        //             const updatedOriginalRows: Array<TableRow> = selectItems(e.target.checked, tableRows, selectedRow, parentRow);
        //             const updatedRows: Array<TableRow> = selectItems(e.target.checked, currentTableRows, selectedRow, parentRow);
        //
        ...
        //         (rows: Array<DataItem<any>>, rowLevel?: number): Array<TableRow> => {
        ...
        //      * @param rowIndex The index of the parent row incase of subRow
        //      */
        //     const selectItems = React.useCallback((checked: boolean, rows: Array<TableRow>, selectedRow: TableRow, parentRow?: TableRow): Array<TableRow> => {
        //         const updatedRows: Array<TableRow> = rows.map((originalRow: TableRow) => {
        //             let updatedSubrows: Array<TableRow> = originalRow.subRows;
        //             if (parentRow && parentRow.rowIndex === originalRow.rowIndex) {
        ...
        //             } else if (originalRow.rowIndex === selectedRow.rowIndex) {
        ...
        return (
            <table className={"table"}>
                <thead>
                    <tr>
                        {headers?.map(({ accessor, sortDirection, ...col }, index: number) => (
                            <th key={index} {...col} onClick={() => onSort(accessor, sortDirection)}>
                                {col.label}
                            </th>
                        ))}
                    </tr>
                ...
                <tbody>
                    {displayRows.map((item, index) => {
                        return <TableRow key={index} {...getRow(item)} columns={headers} />;
                    })}
                </tbody>
                <tfoot>
                    {props.footer && (
                        <tr>
                            <td colSpan={sumCols(props.columns?.length, true, false)}>{props.footer}</td>
                        </tr>
                    )}
                </tfoot>
            </table>
