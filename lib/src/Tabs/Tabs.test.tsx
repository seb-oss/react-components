...
describe("Component: Tabs", () => {
    ...
    describe("Should change active tab when controlled with arrows and select buttons", () => {
        ...
        testList.map((item: keyboardTestUnit) => {
            it(`Testing key [${item.key}]`, () => {
                mountedWrapper.find(".nav-link").at(item.registeredAt).simulate("keydown", { key: item.key });
                expect(mountedWrapper.find(".nav-link").at(item.expectedToChange).hasClass("active")).toBeTruthy();
            });
        });
    });
    ...
    it("Should only change tab with keyboard if active tab is correct", () => {
        ...
        mountedWrapper.find(".nav-link").at(1).simulate("keydown", { key: "enter" });
        expect(mountedWrapper.find(".nav-link").at(1).hasClass("active")).toBeFalsy();
    });
});
