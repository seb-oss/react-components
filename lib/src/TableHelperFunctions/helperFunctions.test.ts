...
describe("Table helper functions:", () => {
    ...
    describe("searchTextByColumns", () => {
        ...
        testCases.map((item: SearchTextByColumnTestCase) => {
            it(item.statement, () => {
                expect(searchTextByColumns(testData, item.keyword, item.fields)).toEqual(item.result);
            });
        });
    });
    ...
    describe("paginate", () => {
        ...
        testCases.map((item: PaginateTestCase) => {
            it(item.statement, () => {
                expect(paginate(item.data, item.offset, item.currentPage)).toEqual(item.result);
            });
        });
    });
    ...
    describe("onRowSelect", () => {
        ...
        testCases.map((item: RowSelectTestCase) => {
            it(item.statement, () => {
                expect(onRowSelect(item.event, item.data, item.rowUniqueAccessor, item.rowId)).toEqual(item.result);
            });
        });
    });
});
